...
* We can use git stash commands as many times as we want. Suppose, i type git stash command thrice, so there will be 3 stash instances 

#### Viewing stashes

    git stash list

* let us view all stashes 
...
* git restore is a brand that helps with undoing operations.

##### Undoing changes in a file

    git restore <file-name>

* To restore the file to the contents in the HEAD, use git restore like above

##### Unstaging a particular file
...
* Suppose you've just made a couple of commits on the master branch, but you actually meant to make them on a seperate branch instead. To undo these commits, you can use git reset.

######

    git reset <commit-hash>  # will reset the repo back to a specific commit. The commits are gone. Yaani itne commit-hash tak rahega code. Will bring those changes in the working directory from the repo
...
* git revert instead creats a brand new commit which revers/undos the changes from a commit. Because it results in a new commit, you will be prompted to enter a commit message.
...
#### Which one to use in case of reverting back the commits?
